...
describe('IngredientService', () => {
  ...
  describe('normalizeIngredientName', () => {
    ...
    it('should normalize "Eldorado Tomater 400g" to "tomater"', () => {
      expect(ingredientService.normalizeIngredientName('Eldorado Tomater 400g')).to.equal(
        'tomater'
      );
    });
  });
  ...
  describe('syncScrapedIngredients', () => {
    it('should handle duplicate products for same ingredient', async () => {
      ...
      expect(milkIngredient.products.map(p => p.store)).to.include.members(['ICA', 'Coop']);
