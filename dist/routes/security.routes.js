...
// Log security event
router.post('/audit/log', auth, rateLimitMiddleware.api(), validateRequest(deviceInfoSchema), asyncHandler(async (req, res) => {
    if (!req.user) {
...
// Get audit logs
router.get('/audit/logs', auth, rateLimitMiddleware.api(), validateRequest(auditLogQuerySchema), asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new ValidationError('Authentication required');
    }
    try {
...
// Get 2FA status
router.get('/2fa/status', auth, rateLimitMiddleware.api(), asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new ValidationError('Authentication required');
    }
    try {
...
// Enable 2FA
router.post('/2fa/enable', auth, rateLimitMiddleware.api(), asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new ValidationError('Authentication required');
    }
    try {
...
// Disable 2FA
router.post('/2fa/disable', auth, rateLimitMiddleware.api(), asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new ValidationError('Authentication required');
    }
    try {
...
// Verify 2FA token
router.post('/2fa/verify', auth, rateLimitMiddleware.api(), validateRequest(z.object({ token: z.string() })), asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new ValidationError('Authentication required');
    }
    try {
        const isValid = await twoFactorService.verify2FA(new ObjectId(req.user.id), req.body.token, {
            type: 'unknown',
            name: 'Unknown',
            os: 'Unknown',
            browser: 'Unknown',
...
// Get active sessions
router.get('/sessions', auth, rateLimitMiddleware.api(), asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new ValidationError('Authentication required');
    }
    try {
...
// Revoke session
router.post('/sessions/revoke', auth, rateLimitMiddleware.api(), validateRequest(z.object({ sessionId: objectIdSchema })), asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new ValidationError('Authentication required');
    }
    try {
        await sessionService.revokeSession(new ObjectId(req.user.id), new ObjectId(req.body.sessionId), {
            type: 'unknown',
            name: 'Unknown',
            os: 'Unknown',
            browser: 'Unknown',
...
router.post('/sessions/revoke-others', auth, rateLimitMiddleware.api(), asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new ValidationError('Authentication required');
    }
    try {
        ...
        await sessionService.revokeOtherSessions(new ObjectId(req.user.id), currentSession._id, {
            type: 'unknown',
            name: 'Unknown',
            os: 'Unknown',
            browser: 'Unknown',
...
router.get('/devices', auth, rateLimitMiddleware.api(), asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new ValidationError('Authentication required');
    }
    try {
...
// Revoke device
router.post('/devices/revoke', auth, rateLimitMiddleware.api(), validateRequest(z.object({ deviceId: z.string() })), asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new ValidationError('Authentication required');
    }
    try {
        await sessionService.revokeDevice(new ObjectId(req.user.id), req.body.deviceId, {
            type: 'unknown',
            name: 'Unknown',
            os: 'Unknown',
            browser: 'Unknown',
...
router.get('/suspicious-activities', auth, validateRequest(getSuspiciousActivitiesSchema, 'query'), async (req, res) => {
    try {
...
router.post('/suspicious-activities/update', auth, validateRequest(updateSuspiciousActivitySchema), async (req, res) => {
    try {
        await securityService.updateActivityStatus(new ObjectId(req.body.activityId), req.body.status, req.body.resolution);
