...
router.post('/register', registerLimiter, [
    ...
    if (inviteCode) {
        await db.collection('invitations').updateOne({ code: inviteCode }, {
            $set: {
                status: 'used',
        ...
        await db.collection('recipe_collections').updateMany({ 'pendingInvites.email': email }, {
            $push: {
                'collaborators': {
                    $each: [{
...
router.post('/signup', [
    ...
    if (invites && invites.length > 0) {
        const invitationService = new InvitationService(db.collection('invitations'), db.collection('users'));
        const invitationPromises = invites.map(async (invite) => {
            try {
                const invitation = await invitationService.createInvitation(userId, invite.email, invite.name);
...
router.post('/accept-invite', [
    ...
    await db.collection('invitations').updateOne({ _id: invitation._id }, {
        $set: {
            status: 'accepted',
            acceptedAt: new Date()
        }
    ...
    // Create token
    const jwtToken = jwt.sign({ id: result.insertedId }, process.env.JWT_SECRET, { expiresIn: '7d' });
    res.status(201).json({
        token: jwtToken,
        user: {
            id: result.insertedId,
...
router.get('/invite/:code', asyncHandler(async (req, res) => {
    ...
    const inviter = await db.collection('users').findOne({ _id: invitation.inviterId }, { projection: { name: 1, email: 1 } });
    res.json({
        invitation: {
            email: invitation.email,
