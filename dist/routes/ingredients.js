...
// Search ingredients
router.get('/search', rateLimiter.api(), async (req, res) => {
    try {
...
router.get('/:id', rateLimiter.api(), async (req, res) => {
    try {
...
router.get('/:id/prices', rateLimiter.api(), async (req, res) => {
    try {
        const db = await connectToDatabase();
        const ingredient = await db.collection('ingredients').findOne({ _id: new ObjectId(req.params.id) }, { projection: { prices: 1, name: 1 } });
        if (!ingredient) {
            return res.status(404).json({ message: 'Ingredient not found' });
        }
...
router.get('/suggestions', rateLimiter.api(), async (req, res) => {
    try {
        const query = req.query.q;
...
// Update ingredient
router.put('/:id', auth, validateRequest(updateIngredientSchema), asyncHandler(async (req, res) => {
    ...
    const result = await db.collection('ingredients').updateOne({ _id: ingredientId }, { $set: updateData });
...
router.post('/', auth, rateLimiter.api(), validateRequest(createIngredientSchema), asyncHandler(async (req, res) => {
    if (req.user?.role !== 'admin') {
        return res.status(403).json({ message: 'Admin access required' });
    }
    const db = await connectToDatabase();
...
// Search ingredients
router.get('/search', validateRequest(searchQuerySchema), asyncHandler(async (req, res) => {
    const authReq = req;
...
router.get('/:ingredientId/prices', asyncHandler(async (req, res) => {
    const ingredient = await ingredientService.getIngredientWithPrices(req.params.ingredientId, req.query.currency);
    return res.json(ingredient);
}));
// Get ingredient with prices
router.get('/:id/with-prices', rateLimiter.api(), async (req, res) => {
    try {
        const db = await connectToDatabase();
        const ingredient = await db.collection('ingredients').findOne({ _id: new ObjectId(req.params.id) }, { projection: { prices: 1, name: 1 } });
        if (!ingredient) {
...
router.get('/suggestions', asyncHandler(async (req, res) => {
    const authReq = req;
    const suggestions = await ingredientService.getAutoCompleteSuggestions(req.query.q, authReq.user?._id?.toString(), req.query.limit ? parseInt(req.query.limit) : undefined);
