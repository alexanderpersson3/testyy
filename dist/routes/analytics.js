...
// Track event
router.post('/events', requireAuth, analyticsLimiter, validateRequest({ body: eventSchema }), async (req, res) => {
    try {
...
// Get user stats
router.get('/user/stats', requireAuth, analyticsLimiter, validateRequest({ query: timeRangeSchema }), async (req, res) => {
    try {
        const db = await getDb();
...
// Get item stats
router.get('/items/:itemId/stats', requireAuth, analyticsLimiter, validateRequest({ query: timeRangeSchema }), async (req, res) => {
    try {
        const db = await getDb();
...
// Get trending items
router.get('/trending', requireAuth, analyticsLimiter, validateRequest({ query: timeRangeSchema }), async (req, res) => {
    try {
        const db = await getDb();
        const { start, end } = req.query;
        const pipeline = [
