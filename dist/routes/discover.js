...
router.get('/recommended', auth, [
    ...
    const db = await connectToDatabase();
    const discoverService = new DiscoverService(db.collection('recipes'));
    const { recipes, total } = await discoverService.getRecommendedRecipes(new ObjectId(req.user.id), req.query);
    res.json({
        recipes,
        total,
        page: parseInt(req.query.page) || 1,
...
router.get('/categories/popular', asyncHandler(async (req, res) => {
    const db = await connectToDatabase();
    const discoverService = new DiscoverService(db.collection('recipes'));
...
router.get('/cuisines/popular', asyncHandler(async (req, res) => {
    const db = await connectToDatabase();
    const discoverService = new DiscoverService(db.collection('recipes'));
