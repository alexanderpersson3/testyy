...
async function hasCollectionAccess(db, collectionId, userId, requiredRole = 'viewer') {
    const collections = db.collection('recipe_collections');
...
router.post('/', auth, [
    ...
    if (order === undefined) {
        const lastCollection = await db.collection('recipe_collections')
            .findOne({ userId: new ObjectId(req.user.id) }, { sort: { order: -1 } });
    ...
    // Log activity
    await getCollectionActivityService().logCollectionActivity(result.insertedId, new ObjectId(req.user.id), 'collection_created', {
...
router.post('/:id/share', auth, [
    ...
    const result = await db.collection('recipe_collections').updateOne({ _id: collectionId }, {
        $push: {
            collaborators: {
                $each: [
                    {
...
router.patch('/:id/share/:userId', auth, [
    check('role').isIn(['viewer', 'editor'])
], asyncHandler(async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
...
router.delete('/:id/share/:userId', auth, asyncHandler(async (req, res) => {
    ...
    const collection = await db.collection('recipe_collections').findOne({ _id: collectionId });
    if (collection && (!collection.collaborators || collection.collaborators.length === 0)) {
        await db.collection('recipe_collections').updateOne({ _id: collectionId }, { $set: { isShared: false } });
    }
    res.json({ success: true });
}));
