...
// Create supplier
router.post('/', auth, validateRequest(createSupplierSchema), async (req, res) => {
    try {
...
// Update supplier
router.patch('/:supplierId', auth, validateRequest(updateSupplierSchema), async (req, res) => {
    try {
        const supplier = await supplierService.updateSupplier(new ObjectId(req.params.supplierId), req.body);
        if (!supplier) {
...
// Search suppliers
router.get('/', validateRequest(searchSuppliersSchema, 'query'), async (req, res) => {
    try {
...
// Add product
router.post('/:supplierId/products', auth, validateRequest(addProductSchema), async (req, res) => {
    try {
        const product = await supplierService.addProduct(new ObjectId(req.params.supplierId), req.body);
...
// Update product
router.patch('/:supplierId/products/:productId', auth, validateRequest(addProductSchema.partial()), async (req, res) => {
    try {
        await supplierService.updateProduct(new ObjectId(req.params.supplierId), new ObjectId(req.params.productId), req.body);
        res.status(204).send();
    }
    catch (error) {
...
// Add special deal
router.post('/:supplierId/deals', auth, validateRequest(addSpecialDealSchema), async (req, res) => {
    try {
        const deal = await supplierService.addSpecialDeal(new ObjectId(req.params.supplierId), {
            ...req.body,
            startDate: new Date(req.body.startDate),
            endDate: new Date(req.body.endDate),
            products: req.body.products.map((id) => new ObjectId(id)),
        });
...
// Add review
router.post('/:supplierId/reviews', auth, validateRequest(addReviewSchema), async (req, res) => {
    try {
...
// Get reviews
router.get('/:supplierId/reviews', validateRequest(getReviewsSchema, 'query'), async (req, res) => {
    try {
...
// Report review
router.post('/reviews/:reviewId/report', auth, validateRequest(reportReviewSchema), async (req, res) => {
    try {
...
// Verify supplier
router.post('/:supplierId/verify', auth, requireRole(UserRole.ADMIN), validateRequest(verifySupplierSchema), async (req, res) => {
    try {
        await supplierService.verifySupplier(new ObjectId(req.params.supplierId), new ObjectId(req.user.id), req.body.status, req.body.document, req.body.notes);
        res.status(204).send();
    }
    catch (error) {
