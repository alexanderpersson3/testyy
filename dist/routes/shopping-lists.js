...
router.post('/from-recipes', auth, [
    ...
    const shoppingList = {
        ...
        recipeIds,
        servingsMultiplier: Object.fromEntries(recipeIds.map((recipeId) => [recipeId.toString(), servingsMultiplier[recipeId.toString()] || 1])),
        isArchived: false,
        createdAt: new Date(),
        updatedAt: new Date()
    };
...
router.post('/:id/items', auth, [
    ...
    const result = await db.collection('shopping_lists').updateOne({ _id: listId, userId }, {
        $push: {
            items: {
                $each: items
            } // Type assertion needed due to MongoDB types limitation
        },
...
router.put('/:id/items/:itemIndex', auth, [
    ...
    const result = await db.collection('shopping_lists').updateOne({ _id: listId, userId }, { $set: updateFields });
    if (result.matchedCount === 0) {
        return res.status(404).json({ message: 'Shopping list not found or unauthorized' });
    }
    res.json({ success: true });
...
router.delete('/:id/items/:itemIndex', auth, asyncHandler(async (req, res) => {
    ...
    await db.collection('shopping_lists').updateOne({ _id: listId }, {
        $pull: {
            items: null // Type assertion needed due to MongoDB types limitation
        }
    });
    res.json({ success: true });
}));
