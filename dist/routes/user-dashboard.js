...
router.get('/', auth, asyncHandler(async (req, res) => {
    ...
    const recentItems = await db.collection('shopping_lists')
        .aggregate([
        { $match: { userId } },
        { $unwind: '$items' },
        {
...
router.patch('/settings', auth, asyncHandler(async (req, res) => {
    ...
    const updateData = Object.fromEntries(Object.entries(req.body)
        .filter(([key]) => allowedSettings.includes(key)));
    await db.collection('user_settings').updateOne({ userId }, {
        $set: updateData,
        $setOnInsert: {
            userId,
            createdAt: new Date()
