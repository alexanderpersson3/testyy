...
router.get('/metrics', auth, isAdmin, rateLimitMiddleware.api(), validateRequest(getMetricsSchema, 'query'), asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new ValidationError('Authentication required');
    }
    try {
        const { startDate, endDate } = validateDates(req.query);
        const metrics = await performanceService.getPerformanceMetrics(new Date(startDate), new Date(endDate));
        res.json(metrics);
    }
    catch (error) {
...
router.get('/metrics/realtime', auth, isAdmin, rateLimitMiddleware.api(), validateRequest(getRealTimeMetricsSchema, 'query'), asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new ValidationError('Authentication required');
    }
    try {
...
//# sourceMappingURL=search-performance.routes.js.map