...
router.post('/', auth, [
    ...
    if (collection.isDefault) {
        await db.collection('recipe_collections').updateMany({ userId }, { $set: { isDefault: false } });
    }
...
router.put('/:id', auth, [
    ...
    if (updateData.isDefault) {
        await db.collection('recipe_collections').updateMany({ userId: collection.userId }, { $set: { isDefault: false } });
    }
    ...
    await db.collection('recipe_collections').updateOne({ _id: collectionId }, { $set: updateData });
...
router.delete('/:id', auth, asyncHandler(async (req, res) => {
    ...
    await db.collection('recipe_collections').deleteOne({ _id: collectionId });
    res.json({ success: true });
}));
...
router.post('/:id/share', auth, [
    ...
    await db.collection('recipe_collections').updateOne({ _id: collectionId }, {
...
router.delete('/:id/share/:userId', auth, asyncHandler(async (req, res) => {
    ...
    const updatedCollection = await db.collection('recipe_collections').findOne({
        _id: collectionId
    });
