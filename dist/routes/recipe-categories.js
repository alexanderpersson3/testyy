...
router.get('/', asyncHandler(async (req, res) => {
    try {
        ...
        const categories = await db.collection('recipe_categories')
            .find({ isActive: true })
            .sort({ order: 1 })
            .toArray();
        return res.json({
...
router.post('/', auth, [
    ...
    try {
        ...
        const result = await db.collection('recipe_categories').insertOne(category);
...
router.put('/:id', auth, [
    ...
    try {
        ...
        const result = await db.collection('recipe_categories').updateOne({ _id: categoryId }, { $set: updateData });
...
// Update collection
router.put('/collections/:id', auth, [
    check('name').optional().trim().notEmpty(),
    check('description').optional().trim(),
    check('isPrivate').optional().isBoolean()
], asyncHandler(async (req, res) => {
    try {
        ...
        const result = await db.collection('recipe_collections').updateOne({ _id: collectionId, userId }, { $set: updateData });
        if (result.matchedCount === 0) {
            return res.status(404).json({
                success: false,
                error: {
...
router.post('/collections/:id/recipes/:recipeId', auth, asyncHandler(async (req, res) => {
    try {
        const db = await connectToDatabase();
        const userId = new ObjectId(req.user.id);
        const collectionId = new ObjectId(req.params.id);
...
router.delete('/collections/:id/recipes/:recipeId', auth, asyncHandler(async (req, res) => {
    try {
        const db = await connectToDatabase();
        const userId = new ObjectId(req.user.id);
        const collectionId = new ObjectId(req.params.id);
        const recipeId = new ObjectId(req.params.recipeId);
        const result = await db.collection('recipe_collections').updateOne({ _id: collectionId, userId }, {
