...
router.get('/', auth, asyncHandler(async (req, res) => {
    ...
    const mealPlans = await db.collection('meal_plans')
...
router.post('/', auth, [
    ...
    const recipeIds = [...new Set(req.body.meals.map((meal) => new ObjectId(meal.recipeId)))];
...
router.put('/:id', auth, [
    ...
    if (req.body.meals) {
        updateData.meals = req.body.meals.map((meal) => ({
            date: new Date(meal.date),
            recipeId: new ObjectId(meal.recipeId),
            servings: meal.servings,
            mealType: meal.mealType,
    ...
    const result = await db.collection('meal_plans').updateOne({ _id: mealPlanId, userId }, { $set: updateData });
...
router.post('/:id/shopping-list', auth, [
    check('startDate').optional().isISO8601(),
    check('endDate').optional().isISO8601(),
    ...
    const shoppingListId = await shoppingListService.generateFromMealPlan(mealPlanId, options);
    res.status(201).json({
        success: true,
        data: {
            shoppingListId
...
router.get('/:id/shopping-list', auth, asyncHandler(async (req, res) => {
    const db = await connectToDatabase();
    const mealPlanId = new ObjectId(req.params.id);
    const userId = new ObjectId(req.user.id);
    ...
    const organizedList = await shoppingListService.getOrganizedList(mealPlan.shoppingListId.toString());
    res.json({
        success: true,
        data: organizedList
    });
...
router.patch('/:id/shopping-list/items', auth, [
    check('items').isArray(),
    ...
    await shoppingListService.updateItems(mealPlan.shoppingListId.toString(), req.body.items);
    res.json({ success: true });
}));
// Archive meal plan
router.post('/:id/archive', auth, asyncHandler(async (req, res) => {
    ...
    const userId = new ObjectId(req.user.id);
    const mealPlanId = new ObjectId(req.params.id);
    const result = await db.collection('meal_plans').updateOne({ _id: mealPlanId, userId }, {
        $set: {
            isArchived: true,
            updatedAt: new Date()
        }
...
router.post('/:id/nutritional-goals', auth, [
    ...
    await db.collection('meal_plans').updateOne({ _id: mealPlanId }, {
        $set: {
...
router.get('/:id/nutritional-warnings/:date', auth, asyncHandler(async (req, res) => {
    const mealPlanId = req.params.id;
...
router.get('/:id/nutritional-summary', auth, asyncHandler(async (req, res) => {
    const db = await connectToDatabase();
    const mealPlanId = new ObjectId(req.params.id);
    const userId = new ObjectId(req.user.id);
    const mealPlan = await db.collection('meal_plans').findOne({
