...
// Create export job
router.post('/export', authenticate, validateRequest(createExportSchema), async (req, res) => {
    try {
        if (!req.user?.id) {
            throw new AuthError('Unauthorized');
        }
        const exportBuffer = await importExportService.exportRecipes(req.body.recipeIds, {
            format: req.body.format,
            ...req.body.options,
        });
        res.setHeader('Content-Type', getContentType(req.body.format));
        res.setHeader('Content-Disposition', `attachment; filename=recipes-export.${req.body.format}`);
...
router.post('/import', authenticate, upload.single('file'), async (req, res) => {
    try {
        ...
        const result = await importExportService.importRecipe(req.user.id, req.file, req.body.format || 'json');
        if (result.success) {
