...
export class MessagingService {
    ...
    async createConversation(creatorId, participantIds, options = {}) {
        ...
        const members = participantObjectIds.map(userId => ({
            userId,
        ...
        await db.collection('conversation_members').insertMany(members);
    ...
    async sendMessage(senderId, conversationId, text) {
        ...
        await db.collection('conversations').updateOne({ _id: new ObjectId(conversationId) }, {
            $set: {
    ...
    async addGroupMembers(conversationId, adminId, newMemberIds) {
        const db = await connectToDatabase();
        // Verify admin rights
        const admin = await db.collection('conversation_members').findOne({
        ...
        await db.collection('conversation_members').insertMany(newMembers);
