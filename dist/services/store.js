export class StoreService {
    ...
    async getStoresByWaitTime(maxWaitTime) {
        const query = maxWaitTime ? { averageWaitTime: { $lte: maxWaitTime } } : {};
        return this.storesCollection.find(query).sort({ averageWaitTime: 1 }).toArray();
    }
    async getStoresWithDeals() {
        const storesWithDeals = await this.dealsCollection.distinct('storeId');
        return this.storesCollection.find({ _id: { $in: storesWithDeals } }).toArray();
    }
    // Product operations
    ...
    async getRelatedProducts(productId) {
        ...
        return this.productsCollection.find({ _id: { $in: product.relatedProducts } }).toArray();
    }
