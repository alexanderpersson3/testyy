...
export class CookingSessionService {
    constructor(wsService, challengeService) {
    ...
    async createSession(userId, data) {
        ...
        const result = await db.collection('cooking_sessions').insertOne(session);
        // Notify followers
     ...
     */
    async getSession(sessionId, currentUserId) {
        const db = await connectToDatabase();
        const session = await db.collection('cooking_sessions').aggregate([
        ...
        if (currentUserId) {
            const like = await db.collection('cooking_session_likes').findOne({
    ...
    async getFeed(params, currentUserId) {
        ...
        const sessions = await db.collection('cooking_sessions').aggregate([
            { $match: query },
            { $sort: { createdAt: -1 } },
            { $skip: offset },
            { $limit: limit },
        ...
        if (currentUserId && sessions.length > 0) {
            ...
            const likes = await db.collection('cooking_session_likes')
                .find({
    ...
    async updateSession(sessionId, userId, data) {
        const db = await connectToDatabase();
        const session = await db.collection('cooking_sessions').findOne({
        ...
        await db.collection('cooking_sessions').updateOne({ _id: new ObjectId(sessionId) }, update);
    }
    /**
    ...
    async deleteSession(sessionId, userId) {
        ...
        await db.collection('cooking_sessions').deleteOne({
            _id: new ObjectId(sessionId)
        });
        // Clean up related data
        await Promise.all([
            db.collection('cooking_session_likes').deleteMany({
                sessionId: new ObjectId(sessionId)
            }),
            db.collection('cooking_session_comments').deleteMany({
                sessionId: new ObjectId(sessionId)
            })
        ]);
    }
    ...
    async likeSession(sessionId, userId) {
        ...
        const existingLike = await db.collection('cooking_session_likes').findOne({
            sessionId: new ObjectId(sessionId),
            userId: new ObjectId(userId)
        });
        if (existingLike) {
        ...
        await Promise.all([
            db.collection('cooking_session_likes').insertOne({
                sessionId: new ObjectId(sessionId),
                userId: new ObjectId(userId),
                createdAt: new Date()
            }),
            db.collection('cooking_sessions').updateOne({ _id: new ObjectId(sessionId) }, { $inc: { likeCount: 1 } })
        ]);
    ...
    async unlikeSession(sessionId, userId) {
        ...
        const result = await db.collection('cooking_session_likes').deleteOne({
            sessionId: new ObjectId(sessionId),
            userId: new ObjectId(userId)
        });
        if (result.deletedCount > 0) {
            await db.collection('cooking_sessions').updateOne({ _id: new ObjectId(sessionId) }, { $inc: { likeCount: -1 } });
        }
    }
    /**
     * Add a comment to a cooking session
     */
    async addComment(sessionId, userId, text) {
        const db = await connectToDatabase();
        const session = await db.collection('cooking_sessions').findOne({
            _id: new ObjectId(sessionId)
        });
        ...
        const result = await db.collection('cooking_session_comments').insertOne(comment);
        await db.collection('cooking_sessions').updateOne({ _id: new ObjectId(sessionId) }, { $inc: { commentCount: 1 } });
        // Notify session owner
        if (!session.userId.equals(new ObjectId(userId))) {
            this.wsService.sendToUser(session.userId.toString(), JSON.stringify({
    ...
    async getComments(sessionId, limit = 50, offset = 0) {
        const db = await connectToDatabase();
        return await db.collection('cooking_session_comments')
