...
async function getActivityHeatmap(db, collectionId) {
    ...
    dailyStats.forEach(stat => {
        ...
        stat.hourly.forEach(hourStat => {
            hourStat.intensity =
                maxCounts.dayHour.total > 0 ? hourStat.count / maxCounts.dayHour.total : 0;
...
export async function getCollectionStats(collectionId) {
    ...
    // Get activity trends
    const [lastDayActivity, lastWeekActivity, lastMonthActivity, dailyTrends, weeklyTrends, monthlyTrends, activityByType, topContributors, recipeStats, collaboratorStats,] = await Promise.all([
        database.collection('collection_activities').countDocuments({
            collectionId,
        ...
        getActivityTrends(database, collectionId, {
            unit: 'day',
            format: '%Y-%m-%d',
            periods: 7,
        }),
        ...
        database
            ...
            .aggregate([
            {
                $match: {
                    collectionId,
                    type: {
                        $in: ['collaborator_added', 'collaborator_removed', 'collaborator_updated'],
                    },
                },
            },
            {
    ...
    const [recipeStatsData] = recipeStats;
    const [collaboratorData] = collaboratorStats;
    // Get activity heatmap
    const heatmap = await getActivityHeatmap(database, collectionId);
    ...
    return {
        ...
        recipeStats: {
            addedCount: recipeStatsData.counts.find((c) => c._id === 'recipe_added')?.count || 0,
            removedCount: recipeStatsData.counts.find((c) => c._id === 'recipe_removed')?.count || 0,
            mostAdded: recipeStatsData.mostAdded.map((r) => ({
                recipeId: r._id,
                title: r.title,
                count: r.count,
            })),
        ...
        collaboratorStats: {
            addedCount: collaboratorData.counts.find((c) => c._id === 'collaborator_added')?.count ||
                0,
            removedCount: collaboratorData.counts.find((c) => c._id === 'collaborator_removed')
                ?.count || 0,
            roleChanges: collaboratorData.counts.find((c) => c._id === 'collaborator_updated')
                ?.count || 0,
...
function calculateGrowthTrend(recipeHistory) {
    ...
    const trend = recipeHistory.length > 0 ? (changes / recipeHistory.length) * 100 : 0;
    return {
        count: changes,
...
function calculateCollaboratorGrowth(collection) {
    ...
    const addedCount = collection.recipeHistory.filter((a) => a.type === 'collaborator_added').length;
    const removedCount = collection.recipeHistory.filter((a) => a.type === 'collaborator_removed').length;
