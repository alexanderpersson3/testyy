...
export class NutritionService {
    ...
    async updateRecipeNutrition(recipeId, userId, nutritionalInfo) {
        await this.ensureInitialized();
        try {
            const result = await this.db.getCollection('recipes').updateOne({ _id: recipeId, 'author._id': userId }, {
                $set: {
                    nutritionalInfo,
                    updatedAt: new Date(),
                },
    ...
    async getMealPlanNutrition(mealPlanId, userId) {
        await this.ensureInitialized();
        try {
    ...
    calculateTotalNutrition(meals, recipes) {
        const total = {
            servingSize: 'total',
            calories: 0,
            protein: 0,
    ...
    calculateDailyAverages(total, days) {
        return {
