...
import { createMockCollection } from './helpers/mockTypes';
describe('SocialService', () => {
    let socialService;
    let mockProfilesCollection;
    let mockFollowsCollection;
    let mockStoriesCollection;
    let mockUsersCollection;
    let mockCommentsCollection;
    let mockReactionsCollection;
    let mockBlocksCollection;
    let mockReportsCollection;
    let mockSharesCollection;
    beforeEach(() => {
    ...
    describe('Profile Management', () => {
        ...
        test('updateProfile should update user profile', async () => {
            ...
            expect(mockProfilesCollection.updateOne).toHaveBeenCalledWith({ userId }, expect.objectContaining({
    ...
    describe('Story Management', () => {
        ...
        test('viewStory should increment view count', async () => {
            ...
            expect(result).toBe(true);
            expect(mockStoriesCollection.updateOne).toHaveBeenCalledWith({ _id: storyId }, { $inc: { views: 1 } });
        });
    });
    describe('Story Interactions', () => {
        ...
        test('addComment should create new comment', async () => {
            ...
            expect(mockCommentsCollection.insertOne).toHaveBeenCalledWith(expect.objectContaining({
                storyId,
                userId,
                content,
                likes: 0
        ...
        test('addReaction should add reaction and remove existing', async () => {
            ...
            expect(mockReactionsCollection.insertOne).toHaveBeenCalledWith(expect.objectContaining({
                storyId,
                userId,
                type
            }));
    ...
    describe('Follow System', () => {
        ...
        test('followUser should create follow relationship', async () => {
            ...
            expect(result).toBe(true);
            expect(mockFollowsCollection.insertOne).toHaveBeenCalledWith(expect.objectContaining({
                followerId,
                followingId
            }));
    ...
    describe('User Safety', () => {
        ...
        test('blockUser should create block and remove follows', async () => {
            ...
            expect(result).toBe(true);
            expect(mockBlocksCollection.insertOne).toHaveBeenCalledWith(expect.objectContaining({
                blockerId,
                blockedId,
                reason: 'Test reason'
            }));
        ...
        test('reportContent should create content report', async () => {
            ...
            const result = await socialService.reportContent(reporterId, 'story', contentId, 'inappropriate', 'Test description');
            expect(result).toEqual(reportId);
            expect(mockReportsCollection.insertOne).toHaveBeenCalledWith(expect.objectContaining({
                reporterId,
                contentType: 'story',
                contentId,
    ...
    describe('Profile Customization', () => {
        ...
        test('getProfileCustomization should return settings', async () => {
            ...
            expect(mockProfilesCollection.findOne).toHaveBeenCalledWith({ userId }, { projection: { customization: 1 } });
        });
    });
});
