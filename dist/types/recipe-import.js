...
import { PDFExtract as PDFExtractBase } from 'pdf.js-extract';
export function validateRecipeData(data) {
    return {
        title: data.title || 'Untitled Recipe',
        description: data.description || '',
        ingredients: data.sections
            .find(s => s.type === 'ingredients')?.content || [],
        instructions: data.sections
            .find(s => s.type === 'instructions')?.content || [],
...
export function extractRecipeData(text) {
    ...
    for (let i = 1; i < Math.min(5, lines.length); i++) {
        if (lines[i].length > 20 && !lines[i].toLowerCase().includes('ingredient') &&
...
function inferDifficulty(text) {
    ...
    if (lowerText.includes('easy') || lowerText.includes('simple') || lowerText.includes('beginner')) {
        return 'easy';
    }
...
function inferCuisine(text) {
    const cuisines = [
        'italian', 'french', 'chinese', 'japanese', 'indian', 'mexican', 'thai',
        'mediterranean', 'greek', 'spanish', 'korean', 'vietnamese'
    ];
    const lowerText = text.toLowerCase();
