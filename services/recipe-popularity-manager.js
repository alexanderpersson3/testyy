...
class RecipePopularityManager {
  ...
  async updateMetrics(recipeId, type, timeSpent = 0) {
    try {
      ...
      await db.collection('recipeMetrics').updateOne(
  ...
  async updatePopularityScores() {
    try {
      ...
      const previousMetrics = await db.collection('recipeMetricsHistory')
        .find({
          timestamp: {
            $gte: yesterday,
            $lt: now
      ...
      const previousMetricsMap = new Map(
        previousMetrics.map(m => [m.recipeId.toString(), m])
      );
