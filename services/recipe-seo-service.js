...
class RecipeSeoService {
  ...
  generateDescription(recipe) {
    const ingredients = recipe.ingredients
      ...
      .map(i => i.name)
      .join(', ');
    return `${recipe.title} - A delicious recipe featuring ${ingredients} and more. ${recipe.description || ''}`.slice(0, 160);
  }
  ...
  async getCanonicalUrl(recipe) {
    ...
    if (recipe.original_recipe_id) {
      const originalRecipe = await getDb().collection('recipes').findOne(
        { _id: new ObjectId(recipe.original_recipe_id) }
      );
      if (originalRecipe) {
