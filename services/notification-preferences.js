...
class NotificationPreferencesManager {
  ...
  async updatePreferences(userId, updates) {
    try {
      ...
      if (updates.digest_frequency && !['never', 'daily', 'weekly'].includes(updates.digest_frequency)) {
      ...
      if (updates.quiet_hours) {
        if (updates.quiet_hours.enabled) {
          ...
          if (!timeRegex.test(updates.quiet_hours.start) || !timeRegex.test(updates.quiet_hours.end)) {
  ...
  async shouldNotify(userId, notificationType) {
    try {
      ...
      if (notificationType === this.NOTIFICATION_TYPES.EMAIL.SECURITY_ALERTS ||
      ...
      if (preferences.quiet_hours.enabled) {
        ...
        const isQuietTime = (currentHour > startHour || (currentHour === startHour && currentMinute >= startMinute)) &&
