...
class UserAnalytics {
  ...
  async getEngagementMetrics(timeframe = '30d') {
    try {
      ...
      const metrics = await db.collection('user_lifecycle_events')
        .aggregate(pipeline)
        .toArray();
  ...
  async getFeatureAdoption(feature, timeframe = '30d') {
    try {
      ...
      const usersWithFeature = await db.collection('user_lifecycle_events').distinct(
        'userId',
        {
          eventType: feature,
          timestamp: { $gte: startDate }
  ...
  async getUserRetention(cohortPeriod = '30d') {
    try {
      ...
      const [result] = await db.collection('user_lifecycle_events')
        .aggregate(pipeline)
        .toArray();
      ...
      return {
        ...
        retentionRate: result ? 
          Math.round((result.retainedUsers / result.totalUsers) * 10000) / 100 : 0
      };
    } catch (err) {
