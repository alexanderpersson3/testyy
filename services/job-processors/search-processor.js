...
class SearchProcessor {
  ...
  async updateSearchStats(data) {
    try {
      ...
      const searchLogs = await db.collection('search_logs')
        .find({
          timestamp: {
      ...
      const stats = {
        ...
        averageResults: searchLogs.reduce((acc, log) => acc + log.resultCount, 0) / searchLogs.length,
        ...
        averageResponseTime: searchLogs.reduce((acc, log) => acc + log.responseTime, 0) / searchLogs.length
      };
