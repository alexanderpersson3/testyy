...
class RecipePreferenceManager {
  ...
  async updatePreferences(userId, {
    ...
    try {
      ...
      const validPreferences = {
        preferredDiets: preferredDiets.filter(diet => 
          this.DIETARY_PREFERENCES.includes(diet)
        ),
        excludedDiets: excludedDiets.filter(diet => 
          this.DIETARY_PREFERENCES.includes(diet)
        ),
        allergens: allergens.filter(allergen => 
          this.ALLERGENS.includes(allergen)
        ),
        cuisinePreferences: cuisinePreferences.filter(pref => 
  ...
  calculateRecipeScore(recipe, preferences) {
    ...
    // Diet match bonus
    const dietMatches = recipe.dietaryTags?.filter(tag => 
      preferences.preferredDiets?.includes(tag)
    ).length || 0;
    score += this.weights.dietMatch * dietMatches;

    // Cuisine match bonus
    const cuisinePreference = preferences.cuisinePreferences?.find(pref => 
      pref.cuisine === recipe.cuisine
    );
   ...
   */
  async getFilteredRecipes(userId, {
    page = 1,
    limit = 20,
    category,
    searchQuery
