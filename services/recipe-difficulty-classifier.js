...
class RecipeDifficultyClassifier {
  constructor() {
    ...
    this.complexityKeywords = {
      ...
      hard: [
        'advanced', 'complex', 'challenging', 'professional',
        'sous vide', 'temper', 'proof', 'knead', 'caramelize',
        'ferment', 'cure', 'smoke', 'confit'
      ]
    };
  }
...
Respond with ONLY ONE WORD: Easy, Medium, or Hard.`;
      ...
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a professional chef who classifies recipe difficulty levels. Respond with exactly one word: Easy, Medium, or Hard.'
          },
          {
            role: 'user',
            content: prompt
  ...
  async classifyRecipe(recipeId, useAI = false) {
    try {
      ...
      // Classify difficulty
      const difficulty = useAI 
        ? await this.classifyWithAI(recipe)
        : this.classifyWithRules(recipe);
