...
class AchievementManager {
  ...
  async trackAchievement(userId, achievementType, value) {
    try {
      ...
      const newBadges = await this.checkAndAwardBadges(userId, achievementType, stats[achievementType]);
  ...
  async getUserBadges(userId) {
    try {
      ...
      return await db.collection('user_badges')
        ...
        .aggregate([
          ...
          },
          {
            $project: {
              rank: {
                $add: [
                  { $indexOfArray: ['$items.userId', new ObjectId(userId)] },
                  1
                ]
              },
              value: {
