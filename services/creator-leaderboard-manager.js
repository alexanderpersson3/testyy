...
class CreatorLeaderboardManager {
  ...
  calculateRisingScore(currentStats, previousStats) {
    ...
    return (
      this.weights.followers * getGrowth(currentStats.followersCount, previousStats.followersCount) +
  ...
  async updateCreatorStats() {
    try {
      ...
      const creators = await db.collection('users').aggregate([
        {
          $lookup: {
            from: 'recipes',
            localField: '_id',
      ...
      const previousStats = await db.collection('creatorStats')
        .find({
          timestamp: {
            $gte: risingThreshold,
            $lt: now
      ...
      const previousStatsMap = new Map(
        previousStats.map(s => [s.userId.toString(), s])
      );
      ...
      const bulkOps = creators.map(creator => {
        const stats = {
          ...
          averageRating: creator.recipes.reduce((acc, r) => acc + (r.averageRating || 0), 0) / creator.recipes.length,
        ...
        const risingScore = previousStat
          ? this.calculateRisingScore(stats, previousStat)
          : 0;
