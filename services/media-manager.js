...
class MediaManager {
      ...
      return {
        ...
        urls: {
          ...
          thumbnails: media.thumbnails ? 
            Object.fromEntries(
              Object.entries(media.thumbnails).map(([size, key]) => [
                size,
                this.getMediaUrl(key)
              ])
            ) : {}
        }
      };
  ...
  async uploadVideo(file, userId, metadata = {}) {
    ...
    try {
      ...
      this.processVideo(result.insertedId, file.buffer, userId, filename)
        .catch(async err => {
      ...
      if (media.type === 'image' && media.thumbnails) {
        ...
        if (media.variants) {
          urls.variants = Object.fromEntries(
            Object.entries(media.variants).map(([quality, key]) => [
              quality,
              this.getMediaUrl(key)
            ])
          );
