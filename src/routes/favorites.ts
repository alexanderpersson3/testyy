...
router.get('/', 
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const favorites = await db.collection<FavoriteItem>('favorite_items')
      .aggregate([
        {
          $match: {
...
router.patch('/:id',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const updateData: Partial<FavoriteItem> = {
      ...req.body,
      updatedAt: new Date()
    };

    if (updateData.defaultQuantity) {
      updateData.defaultQuantity = parseFloat(updateData.defaultQuantity as any);
    }

    const result = await db.collection<FavoriteItem>('favorite_items').updateOne(
      {
        _id: favoriteId,
        userId: new ObjectId(req.user!.id)
      },
...
router.delete('/:itemId',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    await favoritesService.removeFavorite(
      ...
      req.query.itemType as 'recipe' | 'ingredient'
    );
...
router.post('/:id/add-to-list/:listId',
  auth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const db = await connectToDatabase();
    const favoriteId = new ObjectId(req.params.id);
    ...
    // Get favorite item
    const favorite = await db.collection<FavoriteItem>('favorite_items').findOne({
      _id: favoriteId,
      userId: new ObjectId(req.user!.id)
    });
    ...
    await db.collection('shopping_lists').updateOne(
      { _id: listId },
      { 
        $push: { items: { $each: [shoppingItem] } } as any,
...
router.get('/',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const itemType = req.query.itemType as 'recipe' | 'ingredient' | undefined;
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 20;

    if (itemType === 'recipe') {
      const { recipes, total } = await favoritesService.getFavoriteRecipes(
        new ObjectId(req.user!.id),
        page,
        limit
      );
