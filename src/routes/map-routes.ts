...
const searchOptionsSchema = z.object({
  ...
  storeType: z.enum(['supermarket', 'specialty', 'farmers_market', 'convenience']).optional(),
...
router.get('/stores/nearby',
  ...
  async (req, res) => {
    try {
      ...
      const stores = await mapService.findNearbyStores(coordinates, options);
      res.json(stores);
    } catch (error) {
...
router.get('/stores/recipe/:recipeId',
  recipeStoresLimiter,
  validateRequest(z.object({
    params: z.object({
...
router.get('/stores/:storeId/deals',
  dealsLimiter,
  validateRequest(z.object({
...
router.post('/stores/:storeId/deals',
  addDealLimiter,
  validateRequest(z.object({
    params: storeIdSchema,
    body: dealSchema
...
router.post('/stores/:storeId/wait-time',
  waitTimeLimiter,
  validateRequest(z.object({
    params: storeIdSchema,
    body: waitTimeSchema
