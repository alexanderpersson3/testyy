...
router.get('/', auth, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const favorites = await db.collection<FavoriteRecipe>('favorite_recipes')
    .aggregate([
      { 
        $match: { 
...
router.post('/:recipeId',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const existing = await db.collection<FavoriteRecipe>('favorite_recipes').findOne({
      userId: new ObjectId(req.user!.id),
      recipeId
    });
    ...
    const favorite: Omit<FavoriteRecipe, '_id'> = {
      userId: new ObjectId(req.user!.id),
      recipeId,
      note: req.body.note,
      timesCooked: 0,
      rating: req.body.rating,
      personalTags: req.body.personalTags?.map((tag: string) => tag.trim().toLowerCase()) || [],
      createdAt: new Date(),
      updatedAt: new Date()
    };

    const result = await db.collection<FavoriteRecipe>('favorite_recipes').insertOne(favorite as FavoriteRecipe);
...
router.patch('/:id',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const updateData: Partial<FavoriteRecipe> = {
      ...req.body,
      updatedAt: new Date()
    };

    if (updateData.personalTags) {
      updateData.personalTags = updateData.personalTags.map((tag: string) => tag.trim().toLowerCase());
    }
...
router.get('/by-tag/:tag', auth, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const db = await connectToDatabase();
...
router.get('/most-cooked', auth, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const db = await connectToDatabase();
