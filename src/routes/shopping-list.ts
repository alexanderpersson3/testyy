...
import { ShoppingList as DBShoppingList } from '../types/models.js';
...
function getListId(list: ShoppingListData): ObjectId {
  return list._id || new ObjectId();
}

// Update the hasListAccess function
async function hasListAccess(db: Db, listId: ObjectId, userId: ObjectId, requiredRole: 'editor' | 'viewer' = 'viewer'): Promise<boolean> {
  const list = await db.collection<ShoppingListData>('shopping_lists').findOne({ _id: listId });
  if (!list) return false;

  // Owner has full access
  if (list.userId.toString() === userId.toString()) return true;
...
async function getShoppingList(db: Db, listId: ObjectId): Promise<ShoppingListData | null> {
  return await db.collection<ShoppingListData>('shopping_lists').findOne({ _id: listId });
}
...
router.get('/', 
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const lists = await db.collection<ShoppingListData>('shopping_lists')
...
router.post('/', 
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const result = await db.collection<ShoppingListData>('shopping_lists').insertOne(newList);
...
router.get('/:id', auth, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const db = await connectToDatabase();
  const list = await db.collection<ShoppingListData>('shopping_lists').findOne({
  ...
  const items = await db.collection<ShoppingListItem>('shopping_list_items')
...
router.post('/:listId/items',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    await db.collection<ShoppingListData>('shopping_lists').updateOne(
      { _id: listId },
      { 
        $push: { items: newItem },
...
router.patch('/:listId/items/:itemId',
  auth,
  [
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const result = await db.collection<ShoppingListData>('shopping_lists').updateOne(
      { 
        _id: listId,
        'items._id': itemId
      },
...
router.delete('/:listId',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const result = await db.collection<ShoppingListData>('shopping_lists').deleteOne({
      _id: listId,
      userId: new ObjectId(req.user!.id)
    });
...
router.delete('/:id/share/:userId',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const updatedList = await db.collection<ShoppingListData>('shopping_lists').findOne({ _id: listId });
...
router.post('/:listId/items/bulk',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    // Create new items
    const newItems: ShoppingListItem[] = req.body.items.map((item: any) => ({
      listId,
...
router.post('/:listId/save-template',
  auth,
  [
    check('name').notEmpty().trim()
  ],
...
router.post('/from-template/:templateId',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const newList: ShoppingListData = {
      userId: new ObjectId(req.user!.id),
      name: req.body.name,
      items: template.items.map((item: Partial<ShoppingListItem>) => ({
        ...item,
...
router.patch('/:listId/store-preferences',
  auth,
  [
...
router.get('/:listId/price-comparison',
  auth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const db = await connectToDatabase();
    const listId = new ObjectId(req.params.listId);
    ...
    const storePrices: Record<string, { total: number; savings: number; items: any[] }> = {};

    list.items.forEach(item => {
      const ingredient = ingredients.find(i => i._id.equals(item.ingredientId));
      if (ingredient?.priceHistory) {
        // Group prices by store
        const storeGroups = ingredient.priceHistory.reduce((acc: any, price: any) => {
          if (!acc[price.store]) {
            acc[price.store] = [];
          }
        ...
        Object.entries(storeGroups as Record<string, Array<{ date: Date; price: number }>>)
...
router.patch('/:listId/items/bulk', 
  auth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const db = await connectToDatabase();
    const listId = new ObjectId(req.params.listId);
...
router.post('/merge', 
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const lists = await Promise.all(
      listIds.map(id => getShoppingList(db, new ObjectId(id)))
    );
    ...
    const mergedList: ShoppingListData = {
      _id: new ObjectId(),
      userId: new ObjectId(req.user!.id),
    ...
    const result = await db.collection<ShoppingListData>('shopping_lists').insertOne(mergedList);
    mergedList._id = result.insertedId;

    // Optionally delete original lists
    if (req.body.deleteOriginals) {
      await db.collection<ShoppingListData>('shopping_lists').deleteMany({
