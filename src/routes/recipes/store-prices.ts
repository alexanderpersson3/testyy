...
import { RecipeDetails, StorePriceResponse, StorePriceIngredient } from '../../types/recipe.js';
...
const storePricesSchema = z.object({
  ...
  store: z.string().optional().refine(
    (val) => !val || val.split(',').every(s => ['Willys', 'Ica', 'Coop', 'Matspar'].includes(s)),
    'Invalid store name(s)'
  )
});
...
router.get('/',
  rateLimiter.scraping(),
  validateRequest({ params: storePricesSchema }),
  async (req, res) => {
    try {
      const db = await getDb();
      const { recipeId, store } = req.params as z.infer<typeof storePricesSchema>;
      
      // Check cache first
      const cacheKey = `store-prices:${recipeId}:${store || 'all'}`;
      const cachedData = await redis.get(cacheKey);
      if (cachedData) {
        return res.json(JSON.parse(cachedData) as StorePriceResponse);
      }

      // Get recipe details
      const recipe = await db.collection<RecipeDetails>('recipes').findOne({
      ...
      // Calculate totals
      const totalPrice = ingredientsWithPrices.reduce((sum: number, item) => sum + (item.newPrice || 0), 0);
      const oldTotalPrice = ingredientsWithPrices.reduce((sum: number, item) => sum + (item.oldPrice || 0), 0);
      ...
      const response: StorePriceResponse = {
        ...
        data: {
          ...
          storeLogo: storeFilter.length === 1 ? STORE_LOGOS[storeFilter[0] as keyof typeof STORE_LOGOS] : null,
...
router.get('/:recipeId/prices', 
  rateLimiter.scraping(),
  validateRequest({ query: ingredientPricesSchema }),
  async (req: Request<unknown, unknown, unknown, z.infer<typeof ingredientPricesSchema>>, res) => {
    try {
      ...
      const filteredPrices = prices.filter((p: StorePriceIngredient) => p.store && storeFilter.includes(p.store));

      // If Matspar was requested but no prices found, add empty results
      if (storeFilter.includes('Matspar') && !filteredPrices.some((p: StorePriceIngredient) => p.store === 'Matspar')) {
        const matsparIngredients = ingredients.map((ingredient: string) => ({
          name: ingredient,
          amount: 1,
          unit: 'st',
          image: undefined,
      ...
      // Group by store
      const storeGroups = filteredPrices.reduce<Record<string, StorePriceIngredient[]>>((acc, price) => {
        if (!price.store) return acc;
      ...
      const result: StorePriceResult[] = Object.entries(storeGroups).map(([storeName, storePrices]) => {
        ...
        return {
          store: storeName,
          storeLogo: STORE_LOGOS[storeName as keyof typeof STORE_LOGOS] || null,
