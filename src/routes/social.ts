...
import { CreateStoryInput } from '../types/social.js';
...
router.post('/profiles/me/highlights',
  auth,
  [
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const highlightId = await socialService.addHighlight(
      new ObjectId(req.user!.id),
      req.body
    );
...
router.delete('/profiles/me/highlights/:highlightId',
  auth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const db = await connectToDatabase();
    const socialService = new SocialService(db);

    const success = await socialService.removeHighlight(
      new ObjectId(req.user!.id),
      new ObjectId(req.params.highlightId)
    );
...
router.post('/follow/:userId',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const success = await socialService.followUser(
      new ObjectId(req.user!.id),
...
router.delete('/follow/:userId',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const success = await socialService.unfollowUser(
      new ObjectId(req.user!.id),
      new ObjectId(req.params.userId)
    );
...
router.get('/explore',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const content = await socialService.getExploreFeed(
      new ObjectId(req.user!.id),
...
router.get('/popular',
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const users = await socialService.getPopularUsers(
...
router.post('/stories/:storyId/comments',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const commentId = await socialService.addComment(
      new ObjectId(req.params.storyId),
      new ObjectId(req.user!.id),
      req.body.content
    );
...
router.post('/stories/:storyId/reactions',
  auth,
  [
    check('type').isIn(['❤️', '👍', '😂', '😮', '😢', '😡'])
  ],
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const success = await socialService.addReaction(
      new ObjectId(req.params.storyId),
      new ObjectId(req.user!.id),
      req.body.type
    );
...
router.delete('/stories/:storyId/reactions',
  auth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const db = await connectToDatabase();
    const socialService = new SocialService(db);

    const success = await socialService.removeReaction(
      new ObjectId(req.params.storyId),
      new ObjectId(req.user!.id)
    );
...
// Get story reactions
router.get('/stories/:storyId/reactions',
  asyncHandler(async (req: Request, res: Response) => {
    const db = await connectToDatabase();
    const socialService = new SocialService(db);
...
router.post('/stories/:storyId/share',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const shareId = await socialService.shareStory(
      new ObjectId(req.params.storyId),
      new ObjectId(req.user!.id),
      req.body.sharedToId ? new ObjectId(req.body.sharedToId) : undefined,
      req.body.message
    );
...
router.post('/block/:userId',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const success = await socialService.blockUser(
      new ObjectId(req.user!.id),
      new ObjectId(req.params.userId),
      req.body.reason
    );
...
router.delete('/block/:userId',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const success = await socialService.unblockUser(
      new ObjectId(req.user!.id),
      new ObjectId(req.params.userId)
    );
...
router.post('/report',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const reportId = await socialService.reportContent(
      ...
      new ObjectId(req.body.contentId),
      req.body.reason,
...
router.patch('/profiles/me/customization',
  auth,
  [
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const success = await socialService.updateProfileCustomization(
      new ObjectId(req.user!.id),
      req.body
    );
...
router.get('/profiles/:userId/customization',
  asyncHandler(async (req: Request, res: Response) => {
    const db = await connectToDatabase();
    const socialService = new SocialService(db);
