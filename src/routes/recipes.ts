...
import { RecipeSearchResponse, RecipeSuggestion, isRecipeSource } from '../types/search.js';
...
router.get('/search', asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await elasticClient.search<RecipeSearchResponse>({
    index: 'recipes',
    body: {
      query: {
        multi_match: {
...
router.get('/:id',
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const recipe = await db.collection<RecipeDocument>('recipes').findOne({ _id: recipeId });
...
// Update the request interface to include file
...
router.post('/',
  ...
  asyncHandler(async (req: AuthenticatedFileRequest, res: Response) => {
    ...
    const db = await connectToDatabase();
    const result = await db.collection<Recipe>('recipes').insertOne(newRecipe);
...
router.get('/suggestions', asyncHandler(async (req: Request, res: Response) => {
  ...
  const suggestions = result.hits.hits
    .map(hit => {
      ...
      if (!source || !isRecipeSource(source)) return null;
    ...
    .filter((suggestion): suggestion is RecipeSuggestion => suggestion !== null);
...
router.post('/:id/remix',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    // Get original recipe
    const originalRecipe = await db.collection<Recipe>('recipes').findOne({ _id: originalRecipeId });
    if (!originalRecipe) {
      return res.status(404).json({ message: 'Recipe not found' });
    }

    // Create new recipe based on original
    const remixedRecipe: Omit<Recipe, '_id'> = {
      ...originalRecipe,
    ...
    delete (remixedRecipe as any)._id;

    const result = await db.collection<Recipe>('recipes').insertOne(remixedRecipe);
...
router.get('/:id/export-text',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const recipe = await db.collection<Recipe>('recipes').findOne({ _id: recipeId });
    if (!recipe) {
      return res.status(404).json({ message: 'Recipe not found' });
    }
...
router.delete('/cookbook/:recipeId',
  auth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const db = await connectToDatabase();
    const userId = new ObjectId(req.user!.id);
