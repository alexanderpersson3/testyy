...
import { Promotion, Flyer } from '../types/store.js';
...
router.get('/', asyncHandler(async (req: Request, res: Response) => {
  ...
  const promotions = await db.collection<Promotion>('promotions')
    .find(filter)
    .toArray();
...
router.get('/:id', asyncHandler(async (req: Request, res: Response) => {
  const db = await connectToDatabase();
  const promotion = await db.collection<Promotion>('promotions').findOne({
...
router.get('/flyers', asyncHandler(async (req: Request, res: Response) => {
  ...
  const db = await connectToDatabase();
  const flyers = await db.collection<Flyer>('flyers')
    .find(filter)
    .toArray();
...
// Get flyer by ID
router.get('/flyers/:id', asyncHandler(async (req: Request, res: Response) => {
  const db = await connectToDatabase();
  const flyer = await db.collection<Flyer>('flyers').findOne({
    _id: new ObjectId(req.params.id)
  });
...
router.patch('/flyers/:id',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    await db.collection('flyers').updateOne(
      { _id: new ObjectId(req.params.id) },
      { $set: updateData }
    );
