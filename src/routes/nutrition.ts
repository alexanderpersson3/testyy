...
import { Recipe, NutritionalInfo } from '../types/recipe.js';
...
router.get('/meal-plans/:id',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const recipeIds = [...new Set(mealPlan.meals.map((meal: { recipeId: ObjectId }) => meal.recipeId))];
...
function calculateNutritionalInfo(ingredients: Array<{ name: string; amount: number; unit: string }>): NutritionalInfo {
...
function calculateDailyAverages(total: NutritionalInfo, days: number): NutritionalInfo {
  return {
