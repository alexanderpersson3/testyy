...
import { Recipe, RecipeComment, RecipeRating } from '../types/recipe.js';
...
router.get('/:recipeId/comments',
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [comments, total] = await Promise.all([
      db.collection<RecipeComment>('recipe_comments')
        .aggregate([
          {
            $match: { recipeId }
          },
...
router.post('/:recipeId/comments',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const comment: Omit<RecipeComment, '_id'> = {
      recipeId,
      userId,
    ...
    const result = await db.collection<RecipeComment>('recipe_comments').insertOne(comment);
...
router.post('/:recipeId/rate',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (existingRating) {
      ...
      // Add new rating
      const rating: Omit<RecipeRating, '_id'> = {
        recipeId,
        userId,
        rating: req.body.rating,
        review: req.body.review,
      ...
      await db.collection<RecipeRating>('recipe_ratings').insertOne(rating);
...
router.get('/:recipeId/ratings',
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [ratings, total] = await Promise.all([
      db.collection<RecipeRating>('recipe_ratings')
        .aggregate([
          {
            $match: { recipeId }
          },
