...
// Get product details
router.get('/:storeId/products/:productId',
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const db = await connectToDatabase();
...
router.get('/:id/categories/:categoryId/products', asyncHandler(async (req: Request, res: Response) => {
  const db = await connectToDatabase();
  const storeId = new ObjectId(req.params.id);
...
router.get('/:id/promotions', asyncHandler(async (req: Request, res: Response) => {
  const db = await connectToDatabase();
  const storeId = new ObjectId(req.params.id);
...
router.get('/:id/flyer', asyncHandler(async (req: Request, res: Response) => {
  const db = await connectToDatabase();
  const storeId = new ObjectId(req.params.id);
...
// Set price alert
router.post('/:storeId/products/:productId/alert',
  auth,
  [
...
router.post('/:storeId/products/:productId/inventory',
  auth,
  [
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    await db.collection('product_inventory').updateOne(
      { storeId, productId },
      { $set: inventory },
      { upsert: true }
    );
...
router.get('/:storeId/products/:productId/inventory/history',
  auth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const db = await connectToDatabase();
    const storeId = new ObjectId(req.params.storeId);
...
router.post('/:storeId/products/:productId/threshold',
  auth,
  [
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    await db.collection('inventory_thresholds').updateOne(
      { storeId, productId },
      { $set: threshold },
      { upsert: true }
    );
...
router.delete('/shopping-lists/:id',
  auth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const db = await connectToDatabase();
    const userId = new ObjectId(req.user!.id);
...
router.put('/shopping-lists/:id/items/:itemIndex',
  auth,
  [
...
router.delete('/shopping-lists/:id/items/:itemIndex',
  auth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const db = await connectToDatabase();
    const userId = new ObjectId(req.user!.id);
...
router.post('/shopping-lists/from-recipes',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const items = Object.values(ingredients).map((ingredient: any) => ({
      name: ingredient.name,
...
router.get('/shopping-lists/:id/recipe-details',
  auth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const db = await connectToDatabase();
    const userId = new ObjectId(req.user!.id);
    ...
    if (list.generatedFrom?.recipes) {
      const recipeIds = list.generatedFrom.recipes.map((r: any) => new ObjectId(r.id));
      const recipes = await db.collection('recipes')
        .find({ _id: { $in: recipeIds } })
        .project({
          name: 1,
...
// Share shopping list
router.post('/shopping-lists/:id/share',
  auth,
  [
    check('email').isEmail(),
...
router.get('/shopping-lists/shared',
  auth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const db = await connectToDatabase();
    const userId = new ObjectId(req.user!.id);
...
router.delete('/shopping-lists/:id/share/:userId',
  auth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const db = await connectToDatabase();
...
router.get('/shopping-lists/:id/collaborators',
  auth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const db = await connectToDatabase();
    const userId = new ObjectId(req.user!.id);
...
router.put('/shopping-lists/:id/share/:userId',
  auth,
  [
    check('permission').isIn(['view', 'edit'])
  ],
...
router.post('/shopping-lists/:id/comments',
  auth,
  [
...
router.get('/shopping-lists/:id/comments',
  auth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const db = await connectToDatabase();
    const userId = new ObjectId(req.user!.id);
...
router.get('/shopping-lists/:id/route',
  auth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const db = await connectToDatabase();
    const userId = new ObjectId(req.user!.id);
    ...
    const locationMap = new Map(
      productLocations.map(loc => [loc.productId.toString(), loc])
    );

    // Group items by department/aisle
    const itemsByLocation = list.items.reduce((acc: any, item: any) => {
    ...
    const estimatedTime = sortedZones.reduce((total: number, zone: any) => {
      // Base time per zone
...
router.post('/:storeId/products/:productId/location',
  auth,
  [
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    await db.collection('product_locations').updateOne(
      { storeId, productId },
      { $set: location },
      { upsert: true }
    );
...
router.put('/:id/layout',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    await db.collection('store_layouts').updateOne(
      { storeId },
      { $set: layoutData },
      { upsert: true }
    );
...
router.get('/wait-time',
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const maxWaitTime = req.query.maxWaitTime
      ? parseInt(req.query.maxWaitTime as string)
      : undefined;
...
router.get('/:storeId/products',
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { products, total } = await storeService.getProducts(
      new ObjectId(req.params.storeId),
...
router.get('/products/:productId/related',
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const products = await storeService.getRelatedProducts(
...
// Get product deals
router.get('/products/:productId/deals',
  asyncHandler(async (req: Request, res: Response) => {
    const db = await connectToDatabase();
    const storeService = new StoreService(
      db.collection('stores'),
    ...
    const deals = await storeService.getProductDeals(
      new ObjectId(req.params.productId)
    );
...
router.get('/:storeId/categories/:category/subcategories',
  asyncHandler(async (req: Request, res: Response) => {
    const db = await connectToDatabase();
    const storeService = new StoreService(
      db.collection('stores'),
    ...
    const subcategories = await storeService.getSubcategories(
      new ObjectId(req.params.storeId),
      req.params.category
    );
...
router.get('/:storeId/deals',
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { deals, total } = await storeService.getCurrentDeals(
      new ObjectId(req.params.storeId),
      parseInt(req.query.page as string) || 1,
      parseInt(req.query.limit as string) || 20
    );
...
router.get('/deals/best',
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const deals = await storeService.getBestDeals(
