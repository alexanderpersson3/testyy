import type { UnitDefinition, UnitCategory } from '../types/express.js';
import type { UnitConversion } from '../types/express.js';
// Base units for each category
const BASE_UNITS: Record<UnitCategory, string> = {
  volume: 'ml',
  weight: 'g',
  length: 'mm',
  temperature: 'celsius',
...
export class UnitConverter {
  ...
  private conversions: Map<string, UnitConversion[]>;
  ...
  private addConversion(from: string, to: string, factor: number, category: UnitCategory): void {
  ...
  private getBaseUnit(category: UnitCategory): string {
  ...
  round(
    ...
    if (useCommonFractions) {
      ...
      const fraction = COMMON_FRACTIONS.reduce((prev: any, curr: any) => {
  ...
  getSuitableUnit(value: number, currentUnit: string): string | null {
    ...
    const categoryUnits = Array.from(this.unitDefinitions.values())
      ...
      .sort((a: any, b: any) => a.toBase - b.toBase);
