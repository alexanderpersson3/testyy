...
import type { RouteParams } from './express.js';
...
export interface ListParams extends RouteParams {
  id: string;
}

export interface ItemParams extends ListParams {
  itemId: string;
}
...
export type ListStatus = 'active' | 'archived' | 'completed';
...
export type CreateTemplateDTO = Omit<
  BaseShoppingListTemplate,
  'owner' | 'usageCount' | 'lastUsed'
> & {
  storeId?: string;
  items: Array<{
    ingredientId: string;
