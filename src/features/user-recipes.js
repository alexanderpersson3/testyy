...
router.post('/saved-recipes/:recipeId', authenticateToken, rateLimiter.api(), async (req, res) => {
  try {
    const savedRecipe = await userRecipeManager.saveRecipe(req.user.id, req.params.recipeId);
    res.status(201).json({
      success: true,
      data: savedRecipe,
    });
...
// Get saved recipes
router.get('/saved-recipes', authenticateToken, rateLimiter.api(), async (req, res) => {
  try {
...
// Get meal plans
router.get('/meal-plans', authenticateToken, rateLimiter.api(), async (req, res) => {
  try {
...
// Delete meal plan
router.delete('/meal-plans/:mealPlanId', authenticateToken, rateLimiter.api(), async (req, res) => {
  try {
...
router.delete(
  ...
  async (req, res) => {
    try {
      await imageManager.deleteRecipeImages(req.params.recipeId, req.body.imageIds);
      res.json({
        success: true,
