...
router.put(
  ...
  async (req, res) => {
    try {
      const preferences = await recipePreferenceManager.updatePreferences(req.user.id, req.body);
...
router.get(
  ...
  async (req, res) => {
    try {
      const result = await recipePreferenceManager.getFilteredRecipes(req.user.id, req.query);
