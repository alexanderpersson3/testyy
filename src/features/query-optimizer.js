...
class QueryOptimizer {
  constructor() {
    ...
    this.coveredQueries = ['recipes.list', 'users.list', 'ingredients.list'];
  }
    ...
    try {
      ...
      for (const [collection, indexes] of Object.entries(this.indexConfigs)) {
        ...
        for (const index of indexes) {
          try {
            await db.collection(collection).createIndex(index.fields, {
              background: true,
              ...index.options,
            });
  ...
  async analyzeQuery(collection, query, sort = {}, options = {}) {
    try {
      ...
      const isOptimal =
        stats.executionTimeMillis < 100 && stats.totalDocsExamined <= stats.nReturned * 2;

      const couldBeCovered =
        this.coveredQueries.includes(this.identifyQueryType(collection, query)) &&
    ...
    try {
      ...
      if (performance.totalIndexSize > performance.totalDataSize * 0.5) {
        performance.suggestions.push(
          'Index size is over 50% of data size. Consider removing unused indexes.'
        );
      }
