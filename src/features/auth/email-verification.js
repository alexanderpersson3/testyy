...
router.get('/verify', rateLimiter.auth(), async (req, res) => {
  try {
...
router.post('/resend', auth, rateLimiter.auth(), async (req, res) => {
  try {
...
router.put(
  ...
  async (req, res) => {
    try {
      await emailVerificationManager.updateEmail(req.user.id, req.body.newEmail, req.body.password);
