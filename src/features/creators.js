...
// Get creator rank
router.get('/rank/:userId', rateLimiter.api(), async (req, res) => {
  try {
    const db = getDb();
    ...
    const user = await db
      .collection('users')
      .findOne({ _id: userId }, { projection: { creatorScore: 1 } });
...
router.post('/update-stats', authenticateToken, rateLimiter.api(), async (req, res) => {
  try {
