...
class SearchService {
  ...
  async searchRecipes(query, filters = {}, sort = 'relevance', page = 1, limit = 10) {
    ...
    const [countResult] = await db.collection('recipes').aggregate(countPipeline).toArray();
  ...
  async searchCollections(query, userId = null, page = 1, limit = 10) {
    ...
    const collections = await db.collection('collections').aggregate(pipeline).toArray();

    const countPipeline = [...pipeline];
    countPipeline.splice(-2);
    countPipeline.push({ $count: 'total' });
    const [countResult] = await db.collection('collections').aggregate(countPipeline).toArray();
  ...
  async getSearchSuggestions(query, type = 'recipe') {
    ...
    return await db.collection(`${type}s`).aggregate(pipeline).toArray();
  }
}
