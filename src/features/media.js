  ...
  async (req, res) => {
    try {
      ...
      if (req.file.mimetype.startsWith('video/')) {
        result = await mediaManager.uploadVideo(req.file, req.user.id, req.body.metadata || {});
      } else if (req.file.mimetype.startsWith('image/')) {
        result = await mediaManager.uploadImage(req.file, req.user.id, req.body.metadata || {});
      } else {
        return res.status(400).json({
          success: false,
