...
// Routes
router.post('/', authenticateToken, isAdmin, validateRequest(createAdSchema), async (req, res) => {
  try {
...
router.post('/:adId/click', authenticateToken, async (req, res) => {
  try {
...
router.get('/:adId/stats', authenticateToken, isAdmin, async (req, res) => {
  try {
