...
// Start AR session
router.post('/sessions', authenticate, validateRequest(startSessionSchema), async (req: any, res: any) => {
  try {
    if (!req.user) {
    ...
    const session = await arService.startSession(
      new ObjectId(req.user._id),
      recipeId ? new ObjectId(recipeId) : undefined
    );
    res.json(session);
  } catch (error) {
...
// End AR session
router.post('/sessions/:sessionId/end', authenticate, async (req: any, res: any) => {
  try {
...
router.post(
  '/measurements',
  authenticate,
  validateRequest(addMeasurementSchema),
  async (req: any, res: any) => {
    try {
      if (!req.user) {
        return res.status(401).json({ error: 'User not authenticated' });
...
router.post(
  ...
  validateRequest(calibrateSchema),
  async (req: any, res: any) => {
    try {
      if (!req.user) {
        return res.status(401).json({ error: 'User not authenticated' });
      ...
      const calibration = await arService.calibrateCamera(
        new ObjectId(req.user._id),
        req.body.deviceId,
        req.file.buffer
      );
...
// Update AR settings
router.patch('/settings', authenticate, validateRequest(updateSettingsSchema), async (req: any, res: any) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'User not authenticated' });
    }

    const settings = await arService.updateSettings(
      new ObjectId(req.user._id),
