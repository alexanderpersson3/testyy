...
router.post(
  ...
  async (req: any, res: any) => {
    try {
      ...
      const metadata = await videoService.uploadVideoWithMetadata(
        videoBuffer,
...
router.get('/recipe/:recipeId', auth, async (req: any, res: any) => {
  try {
...
router.get('/:videoId', auth, async (req: any, res: any) => {
  try {
...
// Get video metadata
router.get('/:videoId/metadata', auth, async (req: any, res: any) => {
  try {
...
// Chapter routes
router.post('/:videoId/chapters', auth, validate(chapterSchema), async (req: any, res: any) => {
  try {
...
router.patch('/:videoId/chapters/:chapterId', auth, async (req: any, res: any) => {
  try {
...
router.delete('/:videoId/chapters/:chapterId', auth, async (req: any, res: any) => {
  try {
    const { videoId, chapterId } = req.params;
...
// Annotation routes
router.post('/:videoId/annotations', auth, validate(annotationSchema), async (req: any, res: any) => {
  try {
    const { videoId } = req.params;
...
router.patch('/:videoId/annotations/:annotationId', auth, async (req: any, res: any) => {
  try {
...
router.delete('/:videoId/annotations/:annotationId', auth, async (req: any, res: any) => {
  try {
    const { videoId, annotationId } = req.params;
...
router.post(
  '/:videoId/subtitles',
  auth,
  subtitleUpload.single('subtitles'),
  async (req: any, res: any) => {
    try {
      if (!req.file) {
      ...
      const subtitlesUrl = await videoService.uploadSubtitles(
        req.params.videoId,
        subtitlesContent,
