...
router.post(
  '/operations',
  authenticate,
  validateRequest(recordOperationSchema),
...
router.get(
  '/operations/pending',
  authenticate,
  validateRequest(getPendingOperationsSchema, 'query'),
  async (req: any, res: any) => {
    try {
...
router.get(
  '/state',
  authenticate,
  validateRequest(getSyncStateSchema, 'query'),
  async (req: any, res: any) => {
    try {
...
// Update sync state
router.patch('/state', authenticate, validateRequest(updateSyncStateSchema), async (req: any, res: any) => {
  try {
    const state = await syncService.updateSyncState(
      new ObjectId(req.user!.id),
      req.body.deviceId,
...
// Sync changes
router.post('/sync', authenticate, validateRequest(syncChangesSchema), async (req: any, res: any) => {
  try {
    const operations = req.body.operations.map((op: SyncOperationInput) => ({
      ...op,
...
router.get(
  '/changes',
  authenticate,
  validateRequest(getChangesSchema, 'query'),
  async (req: any, res: any) => {
    try {
      const changes = await syncService.getChangesSinceLastSync(
        new ObjectId(req.user!.id),
        req.query.deviceId as string,
        req.query.collections as string[]
      );
      res.json(changes);
    } catch (error) {
