    ...
    if (entityId) {
      switch (type) {
        case 'recipe':
          await db
            .collection('recipes')
            .updateOne({ _id: new ObjectId(entityId) }, { $push: { images: result.insertedId } });
          break;
        case 'profile':
          await db
            .collection('users')
            .updateOne({ _id: new ObjectId(entityId) }, { $set: { avatar: urls.main } });
          break;
        case 'challenge':
          await db
    ...
    if (image.entityId) {
      switch (image.type) {
        case 'recipe':
          await db
            .collection('recipes')
            .updateOne({ _id: image.entityId }, { $pull: { images: imageId } });
          break;
        case 'profile':
          await db
            .collection('users')
            .updateOne({ _id: image.entityId }, { $unset: { avatar: '' } });
          break;
        case 'challenge':
          await db
            .collection('challenge_submissions')
