...
class PriceHistoryManager {
  ...
  async getPriceHistory(
    productId,
    userId,
    { startDate = null, endDate = null, period = this.AGGREGATION_PERIODS.DAILY } = {}
  ) {
    try {
      ...
      const history = await db.collection('price_history').aggregate(pipeline).toArray();

      // Get product details
      const product = await db
        .collection('products')
        .findOne({ _id: new ObjectId(productId) }, { projection: { name: 1, category: 1 } });
