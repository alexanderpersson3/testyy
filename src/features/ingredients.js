...
router.get('/:ingredientId/substitutes', rateLimiter.api(), async (req, res) => {
  try {
...
router.get('/', async (req, res) => {
  try {
    const db = getDb();
    const { page = 1, limit = 20, category, season, search, sort = 'name' } = req.query;
...
router.get('/:id/substitutes', async (req, res) => {
  try {
    ...
    const ingredient = await db
      .collection('ingredients')
      .findOne({ _id: ingredientId }, { projection: { substitutes: 1 } });
