...
const validateIntegration = [
  body('type')
    .isIn(Object.values(integrationsManager.integrationTypes))
...
router.post('/', auth, requireAdmin, validateIntegration, validateRequest, async (req, res) => {
  try {
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const result = await integrationsManager.updateIntegration(req.params.integrationId, updates);
