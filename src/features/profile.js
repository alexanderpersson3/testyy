...
// Get user profile
router.get('/me', authenticateToken, rateLimiter.api(), async (req, res) => {
  try {
...
router.get('/:username', rateLimiter.api(), async (req, res) => {
  try {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const profile = await profileManager.updateProfilePicture(req.user.id, req.file);
...
router.patch(
  ...
  async (req, res) => {
    try {
      const settings = await profileManager.updateVisibilitySettings(req.user.id, req.body);
...
router.post('/me/share', authenticateToken, rateLimiter.api(), async (req, res) => {
  try {
