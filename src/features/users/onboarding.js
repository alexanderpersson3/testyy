...
router.get('/state', auth, rateLimiter.api(), async (req, res) => {
  try {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      await onboardingManager.updateStepProgress(req.user.id, step, status, data);
...
// Get next step
router.get('/next-step', auth, rateLimiter.api(), async (req, res) => {
  try {
    const nextStep = await onboardingManager.getNextStep(req.user.id);
...
// Reset onboarding
router.post('/reset', auth, rateLimiter.api(), async (req, res) => {
  try {
