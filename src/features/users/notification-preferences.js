...
const validatePreferences = [
  ...
  body('quiet_hours.end')
    ...
    .matches(/^([0-1][0-9]|2[0-3]):[0-5][0-9]$/)
    .withMessage('Invalid time format (HH:mm)'),
  body('quiet_hours.timezone').optional().isString().withMessage('Invalid timezone'),
];
...
router.put('/', auth, rateLimiter.api(), validatePreferences, validateRequest, async (req, res) => {
  try {
    const success = await notificationPreferences.updatePreferences(req.user.id, req.body);
...
router.get('/types', auth, rateLimiter.api(), async (req, res) => {
  try {
    res.json({
      success: true,
      data: {
