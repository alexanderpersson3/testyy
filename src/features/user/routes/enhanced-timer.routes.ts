...
router.post(
  '/recipes/:recipeId/timers',
  authenticate,
  validateRequest(recipeParamsSchema, 'params'),
  async (req: any, res: any) => {
    try {
      const timerGroup = await timerService.createTimersFromRecipe(
...
router.get(
  ...
  async (req: any, res: any) => {
    try {
      const groups = await timerService.getTimerGroups(
        new ObjectId(req.user!._id),
        req.query.recipeId ? new ObjectId(req.query.recipeId as string) : undefined
      );
      res.json({ groups });
    } catch (error) {
...
router.post(
  ...
  authenticate,
  validateRequest(timerParamsSchema, 'params'),
  async (req: any, res: any) => {
    try {
...
router.post(
  ...
  authenticate,
  validateRequest(groupParamsSchema, 'params'),
  async (req: any, res: any) => {
    try {
...
router.post(
  ...
  validateRequest(
    z.object({
      timerIds: z.array(
        z.string().refine(val => ObjectId.isValid(val), { message: 'Invalid timer ID format' })
      ),
    }),
    'body'
  ),
  async (req: any, res: any) => {
    try {
      const timers = await timerService.startTimers(
        req.body.timerIds.map((id: string) => new ObjectId(id))
      );
      res.json({ timers });
    } catch (error) {
...
router.post(
  ...
  async (req: any, res: any) => {
    try {
      const timers = await timerService.pauseTimers(
        req.body.timerIds.map((id: string) => new ObjectId(id))
      );
      res.json({ timers });
    } catch (error) {
...
router.post(
  ...
  async (req: any, res: any) => {
    try {
      const timers = await timerService.stopTimers(
        req.body.timerIds.map((id: string) => new ObjectId(id))
      );
      res.json({ timers });
    } catch (error) {
