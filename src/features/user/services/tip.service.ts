...
export class TipService {
  ...
  async getTipsByRecipe(recipeId: string, category?: TipCategory): Promise<Tip[]> {
    const query: { recipeId: ObjectId; category?: TipCategory } = {
    ...
    return this.db.getCollection<Tip>('tips').find(query).sort({ position: 1 }).toArray();
  }

  async updateTipPositions(
    recipeId: string,
    userId: string,
    tipPositions: Array<{ tipId: string; position: number }>
  ): Promise<void> {
    ...
    await this.db.getCollection<Tip>('tips').bulkWrite(operations);
  }
}
