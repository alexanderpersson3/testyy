...
export class SessionService {
  ...
  async createSession(
    ...
    const prefs = await db
      .collection<SecurityPreferences>('security_preferences')
      .findOne({ userId });

    // Check active session limit
    if (prefs?.maxActiveSessions) {
      const activeSessions = await db.collection<UserSession>('user_sessions').countDocuments({
        userId,
        status: 'active',
      });
  ...
  async getSessionByToken(token: string): Promise<UserSession | null> {
    ...
    const session = await db.collection<UserSession>('user_sessions').findOne({
      token,
      status: 'active',
    ...
    if (session) {
      ...
      await db.collection<UserSession>('user_sessions').updateOne(
        { _id: session._id },
        {
          $set: {
            lastActive: new Date(),
   ...
   */
  async getActiveSessions(userId: ObjectId): Promise<UserSession[]> {
    const db = await connectToDatabase();
   ...
   * Revoke a session
   */
  async revokeSession(sessionId: ObjectId, userId: ObjectId, deviceInfo: any): Promise<void> {
    const db = await connectToDatabase();

    const result = await db.collection<UserSession>('user_sessions').updateOne(
      {
        _id: sessionId,
        userId,
        status: 'active',
  ...
  async revokeOtherSessions(
    ...
    const result = await db.collection<UserSession>('user_sessions').updateMany(
      {
        userId,
  ...
  async cleanupExpiredSessions(): Promise<void> {
    ...
    await db.collection<UserSession>('user_sessions').updateMany(
      {
        status: 'active',
  ...
  async authorizeDevice(
    ...
    await db.collection<DeviceAuthorization>('device_authorizations').insertOne(auth);
   ...
   */
  async revokeDevice(userId: ObjectId, deviceId: string, deviceInfo: any): Promise<void> {
    const db = await connectToDatabase();

    const result = await db.collection<DeviceAuthorization>('device_authorizations').deleteOne({
      userId,
      deviceId,
    });
   ...
   */
  async getAuthorizedDevices(userId: ObjectId): Promise<DeviceAuthorization[]> {
    const db = await connectToDatabase();
    return db
      .collection<DeviceAuthorization>('device_authorizations')
      .find({ userId })
      .sort({ lastUsed: -1 })
      .toArray();
  }
