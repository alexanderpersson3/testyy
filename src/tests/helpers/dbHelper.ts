...
export class TestDatabase {
  ...
  // User helpers
  static async createTestUser(data: Partial<{
    email: string;
    name: string;
    isPremium: boolean;
  }> = {}): Promise<any> {
  ...
  static async updateUser(userId: ObjectId, data: Partial<{
    email: string;
    name: string;
    isPremium: boolean;
  }>): Promise<void> {
  ...
  // Profile helpers
  static async createTestProfile(userId: ObjectId): Promise<any> {
    const profile = {
      userId,
  ...
  // Story helpers
  static async createTestStory(userId: ObjectId, data: any = {}): Promise<any> {
    const story = {
      userId,
      type: 'image',
      content: 'Test story',
  ...
  // Store helpers
  static async createTestStore(data: any = {}): Promise<any> {
    const store = {
      name: 'Test Store',
      type: 'supermarket',
      location: {
  ...
  static async updateStore(storeId: ObjectId, updates: any): Promise<void> {
  ...
  // Product helpers
  static async createTestProduct(data: any = {}): Promise<any> {
    const product = {
      name: 'Test Product',
      price: 9.99,
      category: 'test',
