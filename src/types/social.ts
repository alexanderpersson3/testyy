...
export type DocumentWithId<T> = T & { _id: ObjectId };
export type DocumentWithOptionalId<T> = T & { _id?: ObjectId };
...
export type InsertDocument<T extends { _id: ObjectId }> = T;

export type AggregateResult<T> = {
  [K in keyof T]: T[K] extends ObjectId ? string : T[K];
};

export type CreateDocumentWithMeta<T extends { createdAt: Date; updatedAt: Date }> = Omit<T, '_id' | 'createdAt' | 'updatedAt'>;
export type InsertDocumentWithMeta<T extends { _id: ObjectId; createdAt: Date; updatedAt: Date }> = T; 