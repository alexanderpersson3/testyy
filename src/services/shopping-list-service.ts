...
import { MealPlan, ShoppingList, Recipe } from '../types/recipe.js';
...
class ShoppingListService {
   ...
   */
  async generateFromMealPlan(mealPlanId: string, options: {
    startDate?: Date;
    endDate?: Date;
    ...
    if (options.startDate || options.endDate) {
      meals = meals.filter(meal => {
        ...
        if (options.startDate && mealDate < options.startDate) return false;
        if (options.endDate && mealDate > options.endDate) return false;
        return true;
      });
    }
   ...
   */
  async updateItems(shoppingListId: string, updates: Array<{
    name: string;
    amount?: number;
    isChecked?: boolean;
    notes?: string;
   ...
   */
  async getOrganizedList(shoppingListId: string): Promise<{
    ...
    const shoppingList = await db.collection<ShoppingList>('shopping_lists').findOne({
      _id: new ObjectId(shoppingListId)
    });
    ...
    const organized: Record<string, typeof shoppingList.items> = {};
    ...
    const result: Record<string, typeof shoppingList.items> = {};
    Object.keys(this.categories).forEach(category => {
      if (organized[category]?.length > 0) {
        result[category] = organized[category].sort((a, b) => 
