 ...
 */
export async function cropToContent(buffer: Buffer): Promise<Buffer> {
  try {
    // Get image metadata
    const metadata = await sharp(buffer).metadata();
    const { width = 0, height = 0 } = metadata;
...
async function detectSkewAngle(edges: Buffer): Promise<number> {
...
async function detectTextRegions(binary: Buffer): Promise<Region[]> {
  try {
    ...
    const pixels = await sharp(binary)
      .raw()
      .toBuffer();
          ...
          while (queue.length > 0) {
            ...
            for (const [dx, dy] of [[0, 1], [1, 0], [0, -1], [-1, 0]]) {
              ...
              if (nx >= 0 && nx < width && ny >= 0 && ny < height &&
                  pixels[nidx] > 128 && !visited.has(nidx)) {
                queue.push([nx, ny]);
                visited.add(nidx);
                area++;
...
function findMainRegion(regions: Region[]): Region {
...
function findPeaks(profile: number[]): number[] {
  const peaks: number[] = [];
