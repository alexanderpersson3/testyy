...
import { NutritionalInfo, NutritionalGoals, MealPlan, Recipe } from '../types/recipe.js';

class NutritionService {
  /**
   * Calculate daily nutritional summary for a meal plan
   */
  async calculateMealPlanNutrition(mealPlanId: string): Promise<void> {
    ...
    const user = await db.collection('users').findOne(
      { _id: mealPlan.userId },
      { projection: { 'settings.nutritionalGoals': 1 } }
    );

    const nutritionalGoals = user?.settings?.nutritionalGoals || mealPlan.nutritionalGoals;

    // Calculate daily summaries
    const dailySummary: MealPlan['dailyNutritionalSummary'] = {};
   ...
   */
  async updateNutritionalGoals(userId: string, goals: NutritionalGoals): Promise<void> {
    const db = await connectToDatabase();
   ...
   */
  async getDailyNutritionalWarnings(mealPlanId: string, date: string): Promise<string[]> {
    const db = await connectToDatabase();
    const mealPlan = await db.collection<MealPlan>('meal_plans').findOne(
      { _id: new ObjectId(mealPlanId) },
      { projection: { dailyNutritionalSummary: 1 } }
    );
