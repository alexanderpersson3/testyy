...
async function getActivityTrends(
  ...
  timeframe: { unit: string; format: string; periods: number }
...
async function getActivityHeatmap(db: any, collectionId: ObjectId): Promise<ActivityHeatmap> {
  ...
  const maxCounts = {
    hourly: { total: 0, byType: new Map<string, number>() },
    daily: { total: 0, byType: new Map<string, number>() },
    dayHour: { total: 0, byType: new Map<string, number>() }
  };
  ...
  hourlyData.forEach((data: any) => {
    ...
    maxCounts.hourly.byType.set(
      type,
      Math.max(maxCounts.hourly.byType.get(type) || 0, hourTypeStats.count)
    );
    ...
    maxCounts.daily.byType.set(
      type,
      Math.max(maxCounts.daily.byType.get(type) || 0, dayTypeStats.count)
    );
    ...
    maxCounts.dayHour.byType.set(
      type,
      Math.max(maxCounts.dayHour.byType.get(type) || 0, dayHourTypeStats.count)
    );
  });
...
function findPeakTimes(heatmap: ActivityHeatmap) {
...
export async function getCollectionStats(collectionId: ObjectId): Promise<CollectionStats> {
  ...
  return {
    ...
    topContributors: topContributors as CollectionStats['topContributors'],
    activityByType: activityByType as CollectionStats['activityByType'],
    recipeStats: {
      addedCount: recipeStats.counts.find((c: ActivityCount) => c._id === 'recipe_added')?.count || 0,
      removedCount: recipeStats.counts.find((c: ActivityCount) => c._id === 'recipe_removed')?.count || 0,
      mostAdded: recipeStats.mostAdded.map((r: { _id: ObjectId; title: string; count: number }) => ({
        recipeId: r._id,
        title: r.title,
        count: r.count
      }))
    ...
    collaboratorStats: {
      addedCount: collaboratorData.counts.find((c: ActivityCount) => c._id === 'collaborator_added')?.count || 0,
      removedCount: collaboratorData.counts.find((c: ActivityCount) => c._id === 'collaborator_removed')?.count || 0,
      roleChanges: collaboratorData.counts.find((c: ActivityCount) => c._id === 'collaborator_updated')?.count || 0,
      byRole: collaboratorData.roles.map((r: { _id: string; count: number }) => ({
        role: r._id as 'viewer' | 'editor',
        count: r.count
      }))
    },
    activityPatterns: {
...
function calculateGrowthTrend(recipeHistory: any[]): { count: number; trend: number } {
  const changes = recipeHistory.reduce((acc, activity) => {
    if (activity.type === 'recipe_added') acc++;
    if (activity.type === 'recipe_removed') acc--;
    return acc;
  }, 0);
...
function calculateCollaboratorGrowth(collection: any): { count: number; trend: number } {
  const currentCount = collection.collaborators?.length || 0;
  const addedCount = collection.recipeHistory
    .filter((a: any) => a.type === 'collaborator_added')
    .length;
  const removedCount = collection.recipeHistory
    .filter((a: any) => a.type === 'collaborator_removed')
    .length;
