...
describe('MessagingService', () => {
  ...
  describe('createConversation', () => {
    it('should create a new conversation between two users', async () => {
      ...
      expect(conversation?.participants.map((p: ObjectId) => p.toString())).toContain(testUser1Id);
      expect(conversation?.participants.map((p: ObjectId) => p.toString())).toContain(testUser2Id);
      expect(conversation?.isGroup).toBe(false);

      const members = await db.collection<ConversationMember>('conversation_members')
        .find({ conversationId })
        .toArray();
      ...
      expect(wsService.sendToUser).toHaveBeenCalledWith(
        testUser2Id,
        expect.stringContaining('new_conversation')
      );
    });

    it('should return existing conversation for non-group chat', async () => {
      const conversationId1 = await messagingService.createConversation(
        testUser1Id,
        [testUser2Id]
      );

      const conversationId2 = await messagingService.createConversation(
        testUser2Id,
        [testUser1Id]
      );
