...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    // Get user's progress
    const userProgress = await db.collection('user_achievements')
      .find({ userId: new ObjectId(req.user.id) })
      .toArray();

    const progressMap = new Map(
      userProgress.map(p => [p.achievementId.toString(), p])
    );
...
router.get('/user/:userId', authenticateToken, async (req, res) => {
  try {
    ...
    // Get user stats
    const stats = await db.collection('user_achievements')
      .aggregate([
...
router.get('/:id/leaderboard', authenticateToken, async (req, res) => {
  try {
    ...
    const leaderboard = await db.collection('user_achievements')
      .aggregate([
        {
          $match: {
            achievementId,
