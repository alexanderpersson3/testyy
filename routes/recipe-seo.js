...
// Routes
router.put('/:recipeId/seo',
  authenticateToken,
  validateRequest(updateSeoSchema, 'body'),
  async (req, res) => {
    try {
      const recipe = await recipeSeoService.updateRecipeSeo(
        req.params.recipeId,
        req.body
      );
      res.json(recipe);
