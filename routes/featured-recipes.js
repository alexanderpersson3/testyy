...
router.post('/', auth, async (req, res) => {
  try {
    ...
    // Check feature limit
    const activeFeatures = await db.collection('featuredRecipes')
      .countDocuments({
        type,
...
router.get('/', async (req, res) => {
  try {
    ...
    const features = await db.collection('featuredRecipes')
      .aggregate([
        { $match: query },
        { $sort: { priority: -1 } },
        {
...
router.put('/:featureId', auth, async (req, res) => {
  try {
    ...
    await db.collection('featuredRecipes').updateOne(
      { _id: new ObjectId(featureId) },
      update
    );
...
router.get('/stats', auth, async (req, res) => {
  try {
    ...
    const now = new Date();
    const stats = await db.collection('featuredRecipes')
      .aggregate([
        {
          $facet: {
            byType: [
