...
router.post('/', authenticateToken, rateLimiter.medium(), async (req, res) => {
  try {
    ...
    if (shoppingList.source === 'RECIPE' && shoppingList.sourceId) {
      ...
      const mealPlan = await db.collection('meal_plans').aggregate([
        ...
        const items = mealPlan.recipes.reduce((acc, recipe) => {
          recipe.ingredients.forEach(ingredient => {
            const existingItem = acc.find(item => 
              item.ingredientId && item.ingredientId.equals(ingredient.ingredientId)
            );
...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    const query = {
      $or: [
        { userId: new ObjectId(req.user.id) },
        { sharedWith: new ObjectId(req.user.id) }
      ]
    };
