...
router.get('/dashboard', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    // Recipe statistics
    const recipeStats = await db.collection('recipes').aggregate([
      {
        $facet: {
          'total': [{ $count: 'count' }],
          'new': [
    ...
    const engagementStats = await db.collection('recipes').aggregate([
      {
        $facet: {
          'likes': [
            { $match: { 'likes.createdAt': { $gte: startDate } } },
            { $count: 'count' }
          ],
          'comments': [
          ...
          'shares': [
            { $match: { 'shares.createdAt': { $gte: startDate } } },
            { $count: 'count' }
          ]
        }
      }
...
router.get('/users', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    // User demographics
    const demographics = await db.collection('users').aggregate([
      {
        $facet: {
          'byCountry': [
    ...
    // User retention
    const retention = await db.collection('users').aggregate([
      {
        $match: { createdAt: { $gte: startDate } }
      },
      {
      ...
      },
      {
        $project: {
          weeksSinceRegistration: {
            $floor: {
              $divide: [
                { $subtract: ['$lastLoginAt', '$createdAt'] },
                7 * 24 * 60 * 60 * 1000
              ]
            }
          }
...
router.get('/recipes', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const creationStats = await db.collection('recipes').aggregate([
      {
        $match: { createdAt: { $gte: startDate } }
      },
      {
    ...
    // Popular categories
    const categoryStats = await db.collection('recipes').aggregate([
      {
        $group: {
          _id: '$category',
          count: { $sum: 1 },
    ...
    const topRecipes = await db.collection('recipes').aggregate([
      {
        $project: {
          title: 1,
          category: 1,
...
router.get('/search', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const searchVolume = await db.collection('search_logs').aggregate([
      {
    ...
    const popularTerms = await db.collection('search_logs').aggregate([
      {
        $match: { timestamp: { $gte: startDate } }
      },
      {
    ...
    // Search success rate
    const successRate = await db.collection('search_logs').aggregate([
      {
        $match: { timestamp: { $gte: startDate } }
      },
      {
...
router.get('/performance', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    // API response times
    const responseTimes = await db.collection('api_logs').aggregate([
      {
        $match: { timestamp: { $gte: startDate } }
      },
      {
    ...
    // Error analysis
    const errors = await db.collection('error_logs').aggregate([
      {
        $match: { timestamp: { $gte: startDate } }
      },
      {
    ...
    // System load
    const systemLoad = await db.collection('system_metrics').aggregate([
      {
        $match: { timestamp: { $gte: startDate } }
      },
      {
