...
const dealFilterSchema = z.object({
  ...
  sortBy: z.enum(['discountDesc', 'priceAsc', 'priceDesc', 'popularityDesc']).default('discountDesc')
});
...
router.get(
  ...
  async (req, res) => {
    try {
      const result = await storeDealAggregator.getStoreDeals(
        req.params.storeId,
        req.query
      );
...
router.post('/:id/deals', authenticateToken, requireAdmin, rateLimiter.medium(), async (req, res) => {
  try {
    const db = getDb();
    const storeId = new ObjectId(req.params.id);
    ...
    const createdDeal = await db.collection('deals')
      .aggregate([
...
router.post('/:id/prices', authenticateToken, requireAdmin, rateLimiter.medium(), async (req, res) => {
  try {
    const db = getDb();
    const storeId = new ObjectId(req.params.id);
