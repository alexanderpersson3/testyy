...
router.post('/prices/batch', authenticateToken, async (req, res) => {
  try {
    ...
    for (let i = 0; i < prices.length; i += chunkSize) {
      ...
      const previousPrices = await db.collection('price_history')
        .find({
      ...
      for (const price of chunk) {
        ...
        if (!previousPrice || price.price < previousPrice.price) {
          ...
          notifications.push(...alerts.map(alert => ({
            userId: alert.userId,
            type: 'PRICE_ALERT',
  ...
  try {
    ...
    for (const deal of processedDeals) {
      if (deal.ingredients?.length > 0) {
        const priceAlerts = await db.collection('price_alerts')
          .find({
        ...
        notifications.push(...priceAlerts.map(alert => ({
          userId: alert.userId,
          type: 'PRICE_ALERT',
...
router.post('/stores', authenticateToken, async (req, res) => {
  try {
    ...
    if (existingStore) {
      ...
      const locationExists = existingStore.locations.some(loc => 
