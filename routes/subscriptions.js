...
const googlePlayNotificationSchema = z.object({
  message: z.object({
    data: z.string(),
    messageId: z.string(),
...
router.get('/history',
  ...
  async (req, res) => {
    try {
      const db = getDb();
      const logs = await db.collection('subscription_logs')
...
router.post('/trial', authenticateToken, async (req, res) => {
  try {
    ...
    const existingTrial = await db.collection('subscriptions')
      .findOne({ userId, type: 'TRIAL' });
...
router.post('/purchase', authenticateToken, async (req, res) => {
  try {
    ...
    const plan = await db.collection('subscription_plans')
...
router.get('/history', authenticateToken, async (req, res) => {
  try {
    ...
    const total = await db.collection('subscriptions')
...
router.put('/plans/:id', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const result = await db.collection('subscription_plans')
      .findOneAndUpdate(
        { _id: planId },
        {
          $set: {
...
router.get('/status', authenticateToken, async (req, res) => {
  try {
    ...
    const tier = await db.collection('subscription_tiers')
      .findOne({ _id: subscription.tierId });
...
router.get('/history', authenticateToken, async (req, res) => {
  try {
    ...
    const total = await db.collection('user_subscriptions')
...
router.get('/admin/analytics', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const analytics = await db.collection('user_subscriptions').aggregate([
      {
        $facet: {
          'activeSubscriptions': [
            {
