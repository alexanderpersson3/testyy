...
router.put('/:campaignId',
  ...
  async (req, res) => {
    try {
      const success = await campaignService.updateCampaign(
...
router.post('/:campaignId/impression',
  async (req, res) => {
    try {
...
router.post('/:campaignId/conversion',
  authenticateToken,
  validateRequest(recordEventSchema, 'body'),
  async (req, res) => {
    try {
...
router.get('/:id/analytics', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const [participants, completions, engagementStats] = await Promise.all([
      ...
      db.collection('campaign_participants').aggregate([
        { $match: { campaignId } },
        {
          $lookup: {
            from: 'users',
