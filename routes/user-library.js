...
router.patch(
  ...
  async (req, res) => {
    try {
      const list = await recipeListManager.updateList(
        req.user.id,
        req.params.listId,
        req.body
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      const recipe = await savedRecipeManager.saveRecipe(
        req.user.id,
        req.params.recipeId
      );
...
router.delete(
  ...
  async (req, res) => {
    try {
      await savedRecipeManager.removeRecipe(
        req.user.id,
        req.params.recipeId
      );
...
router.get(
  ...
  async (req, res) => {
    try {
      const recipes = await savedRecipeManager.getSavedRecipes(
        req.user.id,
        req.query
      );
...
router.get(
  ...
  async (req, res) => {
    try {
      const status = await savedRecipeManager.getRecipeSaveStatus(
        req.user.id,
        req.params.recipeId
      );
